...
| T42 | Test e2e Playwright : login → création source → upload → rapport | S        | **Done** — rejoué et passant le 2026-08-14 après la refonte visuelle et tous les ajouts depuis (21.9s) | T08, T26    |
...
- T42 : **`apps/web/e2e/golden-path.spec.ts` exécuté réellement contre l'infra complète le 2026-08-12**, puis **rejoué le 2026-08-14** après la refonte visuelle et tous les ajouts depuis (notifications, webhooks, séparateur CSV configurable). Premier passage (2026-08-12) — trois problèmes réels rencontrés et corrigés, documentés en commentaire dans le fichier de test : (1) `import.meta.url` incompatible avec la transpilation CommonJS par défaut de Playwright — remplacé par `__dirname` ; (2) hydratation React pas toujours terminée après `networkidle` en mode `next dev` sur cette machine (compiles à froid observés jusqu'à 65s) — ajout d'un délai explicite (`waitForHydration`) avant toute interaction ; (3) timeout par défaut de Playwright (30s) trop court face à la compilation à la demande d'une route lourde (`/api/ingestions`, qui importe `bullmq`). Rejeu du 2026-08-14 — deux problèmes réels supplémentaires trouvés et corrigés, exactement le genre de régression que ce test est censé attraper : (4) `getByLabel("Mot de passe")` matchait aussi le bouton "Afficher/Masquer le mot de passe" (ajouté depuis, son `aria-label` contient "mot de passe" en sous-chaîne) — corrigé avec `{ exact: true }` ; (5) le timeout par défaut d'une assertion `expect(...)` (5s) est distinct du timeout global du test (90s) et n'en profitait donc pas — ajout de `expect: { timeout: 30_000 }` dans `playwright.config.ts`, plus un timeout dédié (45s) sur l'upload pour couvrir le pire cas observé (compilation à froid de `/api/ingestions` : 17s à elle seule). **1 passed, deux fois de suite (28.4s puis 21.9s)**. Toutes ces causes sont spécifiques au confort du mode développement (`next dev`), pas des bugs applicatifs — une vraie build de production n'a pas ce délai de compilation à la demande.
...
| T46 | Migrations exécutées automatiquement au déploiement      | M        | **Done** — `prisma migrate deploy` lancé au démarrage des conteneurs `web`/`worker` (Dockerfile `CMD`), idempotent, vérifié par build+run local des deux images | T45         |
...
| T52 | Webhooks sortants à la validation d'un fichier                        | C        | Done        | T23         |
